...
**注:** `大きさを(100)%にする`{:class="block3looks"}ブロックは大きさを特定の値に設定し、 `大きさを(10)ずつ変える`{:class="block3looks"}ブロックは値を元の値から変更します。例えば `大きさを`{:class="block3looks"} `10` `ずつ変える`{:class="block3looks"}ブロックは大きさの値に10を加算します。
